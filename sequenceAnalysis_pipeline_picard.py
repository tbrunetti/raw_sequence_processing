...
def main():
	...
	for x in range (1, len(sys.argv)):
		...
		#removes duplicates from a sorted bam file (picard)
		call(["java", "-jar", "picard-tools-1.119/MarkDuplicates.jar", 'INPUT=sorted'+str(sys.argv[x])+'.bam', 'OUTPUT=noDup-sorted-'+str(sys.argv[x])+'.bam', 'METRICS_FILE=metrics_picard_noDups'+str(sys.argv[x])+'.txt', 'ASSUME_SORTED=true', 'REMOVE_DUPLICATES=true'])
		print "Duplicates have been removed!"

		#removes duplicates from a sorted bam file (samtools)
		#call(["./samtools-0.1.19/samtools", "rmdup",  'sorted'+str(sys.argv[x])+'.bam', 'noDup-sorted'+str(sys.argv[x])+'.bam'])
		
		print "sorting bam by name..."
		...
		f3=open('finalSort-dups-removed-by-picard'+str(sys.argv[x])+'.bed', 'w')
